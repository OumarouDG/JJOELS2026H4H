        ...
        <div className="flex items-center gap-3">
          <div className="text-right">
            ...
            <div className="text-xs font-bold text-emerald-500 flex items-center gap-1 justify-end">
              <span className="h-1.5 w-1.5 rounded-full bg-emerald-500 animate-pulse" /> Node Active
            </div>
          </div>
        ...
        <div className="lg:col-span-7 flex flex-col gap-8">
          <div className="rounded-2xl border border-slate-200 bg-white p-8 shadow-[0_4px_20px_rgb(0,0,0,0.03)]">
            <div className="mb-6 flex items-center justify-between">
              <h2 className="text-xs font-black uppercase tracking-widest text-slate-400">Control Interface</h2>
          ...
          <div className="sticky top-8 rounded-2xl border border-slate-200 bg-white p-8 shadow-[0_8px_30px_rgb(0,0,0,0.04)] min-h-[450px] flex flex-col justify-center">
            ...
            {!result ? (
              <div className="py-20 text-center border-2 border-dashed border-slate-100 rounded-2xl bg-slate-50/50">
                <p className="text-sm text-slate-400 font-medium">Awaiting biometric signal...</p>
              </div>
            ) : (
              <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
                ...
                <div className="flex items-center gap-6">
                  <div className="text-7xl font-black tracking-tighter text-indigo-600">
                    {Math.round((result.confidence ?? 0) * 100)}<span className="text-2xl ml-1 text-indigo-300">%</span>
                  </div>
                  <div className="h-14 w-[1px] bg-slate-200" />
                  <div>
                    <div className="text-[10px] font-black uppercase tracking-wider text-slate-400 mb-1">Confidence</div>
                    <div className="text-xs font-bold text-slate-500 leading-tight">
                      Neural Pattern<br/>Validated
                    </div>
                  </div>
                </div>

                {/* Status Classification Card */}
                <div className={`relative overflow-hidden p-6 rounded-2xl border-l-[12px] transition-all ${
                  ...
                  {/* Backdrop Glow */}
                  <div className={`absolute -right-8 -top-8 h-32 w-32 rounded-full blur-3xl opacity-30 ${
                    result.prediction?.toLowerCase() === 'healthy' ? 'bg-emerald-400' : 'bg-amber-400'
                  }`} />
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  {Object.entries(result.features || {}).map(([key, val]) => (
                    <div key={key} className="bg-slate-50/80 p-4 rounded-xl border border-slate-100 group hover:border-indigo-200 transition-all duration-300">
                      <div className="text-[9px] uppercase font-black text-slate-400 tracking-wider group-hover:text-indigo-400">{key}</div>
                      <div className="text-base font-mono font-bold text-slate-700 mt-1">{val}</div>
                    </div>
                  ))}
                </div>
              </div>
