...
-Libraries: pyserial, numpy, time, collections
...
--POST /train (runs training on public VOC dataset or loads pre-trained artifact)
--GET /metrics (accuracy, confusion matrix, feature list)
--POST /capture (accepts captured window raw + features, returns “ready for inference” payload)
--POST /infer (accepts features, returns prediction + confidence)
--GET /captures (returns timeline list)
